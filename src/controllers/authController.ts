    ...
    public async login(req: Request, res: Response){
        ...
        for(let user of users) {
            if(await utils.checkPassword(password, user.password)){
                const token = jwt.sign({cveUsuario : user.cveUsuario, username, mascota : user.nombreMascota}, secretKey.jwtSecret, {expiresIn : '1h'});
                return res.json({ message : "OK", token, cveUsuario : user.cveUsuario, username, mascota: user.nombreMascota, nombre: user.nombre, apellidos: user.apellidos, raza: user.nomRaza, descripcion: user.descripcion });
            } else {
